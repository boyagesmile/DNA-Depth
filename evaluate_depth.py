...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        encoder = networks.EfficientEncoder(valid_models=opt.valid_models, pretrained=False,
                                            height=opt.height, width=opt.width)
        depth_decoder = networks.EfficientDecoder(encoder.num_ch_enc)

        model_dict = encoder.state_dict()
        encoder.load_state_dict({k: v for k, v in encoder_dict.items() if k in model_dict})
        depth_decoder.load_state_dict(torch.load(decoder_path, map_location='cuda'))
